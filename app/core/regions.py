...
class RegionDocument:
    ...
    fetch_format: str  # "html" или "pdf"
    # НПА могут утрачивать силу и заменяться новыми — автоматически это не
    # проверяется (для этого нужна платная интеграция с КонсультантПлюс/Гарант
    # или регулярный ручной мониторинг). Дата ниже — когда мы вручную
    # убедились, что документ действующий; её показываем пользователю, чтобы
    # он понимал, насколько свежая это проверка, а не выдавали её за гарантию.
    last_verified: str


# Пилотные регионы для сравнения РНГП. Чтобы добавить новый регион,
# достаточно найти актуальный текст норматива и дописать запись сюда —
# остальной пайплайн (ingestion, retrieval, бот) подхватит её автоматически.
REGIONS: dict[str, RegionDocument] = {
    ...
    "krasnodar_krai": RegionDocument(
        ...
        # docs.cntd.ru подгружает документ по частям при скролле и не отдаётся целиком
        # простым HTTP-запросом, поэтому в качестве источника используем .docx-копию
        # того же приказа, которую администрация Новороссийска публикует у себя
        # (см. страницу ниже, раздел "Нормативы градостроительного проектирования").
    ...
    # Ленинградскую область сознательно не включаю: единственный найденный
    # бесплатный источник (leningr-gov.ru/doc/73484) содержит только само
    # постановление ("Утвердить прилагаемые нормативы... (не приводится)"),
    # а не текст приложения с расчётными показателями — ingestion по нему
    # даёт 5 бесполезных чанков вместо реальных норм. Полный текст приложения
    # есть на docs.cntd.ru (не отдаёт страницу ботам, как и было с Краснодаром
    # изначально) и в Гаранте/КонсультантПлюс (платный доступ). Если найдётся
    # бесплатное зеркало с полным текстом — можно вернуть.
    "sverdlovsk_oblast": RegionDocument(
        ...
        # старое постановление 2010 г. N 380-ПП утратило силу с 01.02.2024 — наглядный
        # пример того, почему дату ручной проверки актуальности важно показывать честно.
...
# Базовый федеральный уровень — действует по всей России и региональные акты
# его только уточняют/дополняют (см. п. 1.1 самого СП: "требования
# конкретизируются... в региональных и местных нормативах"). В REGIONS его не
# кладём: это не альтернатива для сравнения, а фоновый слой, который бот
# подмешивает к любому региону автоматически (см. app/agent/nodes.py).
FEDERAL_DOCUMENT = RegionDocument(
    ...
    # тот же домен, что уже используем для Московской области и Татарстана —
    # meganorm.ru отдаёт полный HTML без JS-подгрузки, в отличие от docs.cntd.ru
...
def all_documents() -> dict[str, RegionDocument]:
    """REGIONS + федеральный документ — используется там, где нужно пройтись
    по всем источникам целиком (ingestion, чистка устаревших данных и т.п.).
    Для UI (выбор региона) федеральный уровень не нужен — там берём REGIONS."""
