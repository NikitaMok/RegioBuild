...
def friendly_llm_failure(exc: BaseException, *, mode: str = "info") -> str:
    ...
    if isinstance(exc, LLMParsingError):
        what_list = (
            "сравнительный перечень требований"
            if mode == "compare"
            else "перечень требований"
        )
        return (
            f"Не удалось устойчиво сформировать {what_list} по этому запросу. "
            "Попробуйте переформулировать запрос или уточнить тип объекта "
            "(например: торговый центр, автосалон)."
        )
