...
по размещению бизнес-объектов. Пиши профессиональным юридическим языком:
...
Сравни нормативы двух регионов для указанного типа бизнес-объекта.
Пиши профессиональным юридическим языком, без разговорных и айтишных
формулировок.

Помимо региональных актов даны федеральные фрагменты (СП 42.13330.2016 и др.).

ГЛАВНОЕ: каждое различие должно опираться на пункт НПА.
...
12. Если citation содержит «123-ФЗ» или «СанПиН» — source_level="федеральный";
    не выдавай статьи 123-ФЗ как региональные требования субъекта.
13. Ответ — строго JSON по схеме, без текста вне JSON. Заполни JSON полностью,
    не обрывай строки на полуслове.
"""
