...
# Держим в синхроне с Settings.requirement_categories — категорий здесь не так
# много, чтобы тащить это через динамический Literal из конфига, а жёсткая
# проверка на уровне типов нужна, чтобы LLM не могла придумать свою категорию.
# "иные_требования" — отдельный catch-all для тем, которые не укладываются в
# первые четыре (парковка, пешеходные потоки, благоустройство и т.п.). Раньше
# такие темы падали в "состав_проекта", и в чате это выглядело странно —
# заголовок "Состав проекта" над требованиями к парковке сбивал с толку.
...
# СП 42.13330.2016 действует на всю страну, региональные акты его только
# уточняют — источник важно показать пользователю отдельно от регионального,
# иначе непонятно, откуда взялась норма и почему она не в тексте регионального НПА.
...
class RequirementItem(BaseModel):
    ...
    # без этого флага пользователь не может понять, читает он норму именно про
    # свой бизнес или общее правило градостроительного проектирования, которое
    # просто оказалось применимо — а разница для него принципиальная
...
class DifferenceItem(BaseModel):
    category: RequirementCategory
    # раньше эти два поля были без description, и модель иногда вместо реальной
    # формулировки писала пустое "в регионе B установлены требования" — теперь
    # явно требуем конкретику
