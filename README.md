...
случае лишь демонстрационный канал. Ядро продукта — HTTP API (`/info`,
`/compare`, `/api/v1/*`, `/health`, `/metrics`). Внешние системы подключаются
независимо от мессенджера.
...
Полученные ответы не заменяют проектную документацию и/или заключение юриста,
а также самостоятельную проверку актуальности нормативов на момент запроса.  
Муниципальные ПЗЗ в индекс не включены, поэтому на сегодняшний день их нужно
проверять отдельно.  
