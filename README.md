...
Прод — **VPS Aeza** (Москва): nginx, API, Telegram-бот, Prometheus/Alertmanager,
еженедельный бэкап. FastAPI + aiogram 3 + Docker; один образ, роль процесса —
`SERVICE_ROLE=api|bot`. Векторный контур — Qdrant Cloud; embeddings — ONNX
(fastembed), без PyTorch в runtime. Дисковый кэш LLM — чтобы не оплачивать
...
Текущий прогон retrieval (`fastembed` + Qdrant, полный корпус; без подстановки
curated-якорей в контекст и без поиска только по curated):
...
(за рамками текущих критериев готовности).
...
| `RU-FED` | Федеральный слой: ГрК РФ, СП 42, 123-ФЗ, СанПиН (полный разбор PDF в индексе) |
...
- прод на VPS Aeza (nginx, бэкапы, мониторинг)  
